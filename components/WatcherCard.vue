<template>
  <div
    class="relative border transition-all duration-500 p-4 flex items-center justify-between gap-4"
  ...
  >
    ...
    <div class="flex items-center gap-3 min-w-0">
      <span :class="watcher.status === 'live' ? 'live-dot' : 'waiting-dot'" />
      <div class="min-w-0">
        <p class="font-mono text-xs text-ghost uppercase tracking-widest truncate">
    ...
    <div class="flex items-center gap-2 shrink-0">
      <a
        ...
        target="_blank"
        class="font-display text-sm tracking-widest px-3 py-1.5 bg-live text-void hover:bg-snow transition-colors"
      >
        TONTON
      </a>
      <button
        @click="$emit('remove', watcher.watchId)"
        class="font-mono text-xs text-ghost hover:text-signal transition-colors px-2 py-1.5"
        title="Hapus"
      >
        ✕
      </button>
