  ...
  >
    <div class="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
      ...
      <div class="min-w-0">
        ...
        <p class="font-mono text-[10px] sm:text-xs mt-0.5"
          ...
          {{ watcher.status === 'live' ? '● LIVE — redirecting...' : '○ waiting...' }}
        </p>
      </div>
    </div>
    <div class="flex items-center gap-1.5 sm:gap-2 shrink-0">
      ...
      <button
        ...
        class="font-mono text-xs text-ghost hover:text-signal transition-colors flex items-center justify-center"
    ...
    <div v-if="watcher.status === 'live'" class="absolute inset-0 pointer-events-none"
      style="background:linear-gradient(90deg,transparent 0%,rgba(0,255,135,0.03) 50%,transparent 100%);animation:shimmer 2s ease-in-out infinite;" />
  </div>
</template>
