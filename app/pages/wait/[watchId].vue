...
<template>
  <main class="min-h-screen text-snow flex flex-col relative overflow-hidden safe-paddings">
    ...
    <header v-if="!isTheater && !isIntro" class="relative z-10 w-full max-w-[1400px] mx-auto p-4 sm:p-6 flex items-center justify-between glass-panel mt-4 rounded-2xl animate-fade-down landscape-hidden">
    ...
    <div class="flex-1 flex flex-col items-center justify-center p-4 sm:p-6 relative z-10 landscape-condensed">
      
      <div v-if="isIntro" class="flex flex-col items-center animate-intro-zoom w-full">
        <div class="w-20 h-24 sm:w-24 sm:h-24 bg-live rounded-full flex items-center justify-center shadow-[0_0_50px_rgba(0,255,135,0.4)] animate-live-pulse mb-6 sm:mb-8">
          <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="black" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polygon points="5 3 19 12 5 21 5 3"/></svg>
        </div>
        <h2 class="font-display text-5xl sm:text-8xl tracking-widest text-live mb-4 text-center">LIVE NOW</h2>
        <p class="font-mono text-[10px] sm:text-xs text-live/60 tracking-widest uppercase animate-pulse">Establishing Signal</p>
      </div>
      ...
      <div v-show="isLive && !isIntro" class="w-full max-w-6xl aspect-video relative transition-all duration-1000 transform" 
        ...
        <div class="absolute top-4 right-4 flex gap-2">
          <button @click="isTheater = !isTheater" class="bg-black/60 hover:bg-black/80 p-2 rounded-lg border border-white/10 backdrop-blur-md transition-colors">
      ...
      <div v-if="!isLive && !isIntro" class="flex flex-col items-center w-full max-w-4xl">
        <div class="glass-panel px-5 py-2 rounded-full mb-6 sm:mb-8 animate-fade-up">
          <p class="font-mono text-[9px] sm:text-xs text-ghost tracking-widest3 uppercase truncate max-w-[250px] sm:max-w-none">
        ...
        <div class="relative mb-8 sm:mb-12 animate-fade-up" style="animation-delay:0.1s;opacity:0">
          <template v-if="!isError">
            <div class="font-display leading-none text-snow opacity-90"
              style="font-size: clamp(2.5rem, 15vw, 9rem);">
              STANDBY
            </div>
            <div class="mt-6 sm:mt-8 flex items-center justify-center gap-3">
              <span v-for="i in 3" :key="i"
                class="inline-block w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-signal shadow-[0_0_10px_rgba(255,45,45,0.5)]"
                :style="`animation: dotPulse 1.5s ease-in-out ${(i-1)*0.3}s infinite`" />
            </div>
          </template>

          <template v-else>
            <div class="font-display leading-none text-signal" style="font-size: clamp(2rem, 12vw, 7rem);">OFFLINE</div>
            <p class="mt-4 sm:mt-6 font-mono text-[10px] sm:text-xs text-ghost max-w-xs mx-auto glass-panel p-4 rounded-2xl">
              Connection lost or session expired.
            </p>
          </template>
        </div>

        <div class="mt-8 sm:mt-12 font-mono text-[9px] sm:text-[10px] text-mist animate-fade-up space-y-2 text-center" style="animation-delay:0.4s;opacity:0">
          <p class="tracking-widest uppercase">Uptime: {{ elapsedStr }}</p>
          <p class="opacity-50 hidden xs:block">Sentinel monitoring active via distributed client IP</p>
        </div>

        <NuxtLink to="/" class="mt-8 sm:mt-10 font-mono text-[9px] sm:text-[10px] text-ghost hover:text-snow transition-all tracking-widest uppercase border-b border-ghost/20 hover:border-snow pb-1">
          ← Return
        </NuxtLink>
      </div>
    ...
    <footer v-if="!isTheater && !isIntro" class="relative z-10 w-full max-w-[1400px] mx-auto m-4 rounded-2xl glass-panel p-4 sm:p-6 animate-fade-up landscape-hidden">
      <div class="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
        <div class="flex items-center gap-4 sm:gap-6">
          <div class="flex items-center gap-2.5">
            ...
            <span class="font-mono text-[9px] text-ghost tracking-widest uppercase">
              {{ isLive ? 'Live' : 'Sentinel' }}
            </span>
          </div>
          ...
          <button v-if="!isLive" @click="toggleRedirect" class="flex items-center gap-2 group cursor-pointer">
            ...
            <span class="font-mono text-[8px] text-ghost group-hover:text-snow uppercase tracking-widest transition-colors">
              Auto-Redirect: {{ autoRedirect ? 'ON' : 'OFF' }}
            </span>
          </button>

          <a v-else :href="videoUrl" target="_blank" class="flex items-center gap-2 group cursor-pointer">
            <span class="font-mono text-[9px] text-live group-hover:text-snow uppercase tracking-widest transition-colors flex items-center gap-2">
              YouTube
              <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="4"/></svg>
            </span>
          </a>
        </div>
        
        <div class="font-mono text-[9px] text-mist tracking-widest uppercase flex items-center gap-4">
          <span class="text-ghost">v5.0.0</span>
        </div>
      </div>
    </footer>
